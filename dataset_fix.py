...
_COLS = ["solar_hourly_watt_generation_house", "hourly_load_kw_house", "hourly_load_watt_house"]
...
def _update_excess(df: pd.DataFrame, idx):
    df.loc[idx, "Excess_energy_watt"] = (
        df.loc[idx, "hourly_load_watt_house"] - df.loc[idx, "solar_hourly_watt_generation_house"]
    )
    ...
    for houses in all_data.values():
        for df in houses.values():
            ...
            if m_range.any():
                ...
                _update_excess(df, df.index[m_range])
                fixed += int(m_range.sum())
    if verbose:
        print(f"🔧 fixed {fixed} row(s)")
    return all_data


def write_fixed_data(
    ...
    if verbose:
        print(f"✅ updated {count} CSV file(s)")
    return count


if __name__ == "__main__":
    from energynet.data_loader import read_all_generated_data
